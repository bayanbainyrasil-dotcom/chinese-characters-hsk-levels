...
> ## Что уже сделано (31 августа 2026)
>
> Всё, описанное ниже, **уже выполнено** для этого сайта. Инструкция оставлена
> на случай, если проект придётся пересоздавать с нуля.
>
> | Параметр | Значение |
> |---|---|
> | Проект | `hsk-bishun` |
> | Ссылка | `https://rnrjvgheyehywarjrwsf.supabase.co` |
> | Регион | Central EU (Frankfurt) |
> | Тариф | Free |
> | Ключ во frontend | `sb_publishable_…` (публичный, лежит в `js/config.js`) |
> | Схема базы | накатана, RLS включён на всех 6 таблицах |
> | Edge Functions | `admin-edits`, `admin-auth`, `tts` — развёрнуты |
> | Site URL | `https://bayanbainyrasil-dotcom.github.io/chinese-characters-hsk-levels/` |
> | Способы входа | почта + пароль, ссылка на почту. **Google выключен** |
> | Подтверждение почты | **выключено** — см. «Почему» ниже |
>
> ### Почему выключено подтверждение почты
>
> У бесплатного Supabase собственный почтовый сервис жёстко ограничен и
> писем практически не доставляет. С включённым подтверждением регистрация
> зависала бы на «проверьте почту», куда письмо не приходит. Обратная сторона:
> адрес не проверяется. Для тренажёра письма это приемлемо — аккаунт нужен
> только для синхронизации прогресса, а доступ к чужим данным закрыт
> политиками RLS. Если подключить свою почтовую службу (Settings → Auth →
> SMTP), подтверждение стоит включить обратно.
>
> ### Чего не хватает для кнопки Google
>
> Нужен OAuth Client ID в Google Cloud. Google с 13 мая 2025 не пускает в
> консоль без двухфакторной аутентификации на аккаунте. Пока провайдер
> выключен, приложение само прячет кнопку Google — она не висит нерабочей.
> Как только провайдер включат в Supabase, кнопка появится без правок кода.
