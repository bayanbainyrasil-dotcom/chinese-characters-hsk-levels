...
> | Озвучка | работает: `TTS_PROVIDER=edge`, записи кэшируются в Storage `tts-audio` |
...
>
> ### Какой движок озвучки работает на самом деле
>
> В браузере пользователя нет ни одного китайского голоса (`speechSynthesis`
> отдаёт 5 голосов, ни одного `zh-*`), поэтому запасной вариант «читает
> браузер» там не работает вовсе — кнопки озвучки молчали бы. Синтез вынесен
> в Edge Function, потому что оба эндпоинта синтеза закрыты и в контейнере
> сборки, и в сети пользователя.
>
> Функция сначала пробует нейронные голоса Microsoft Edge. **Сейчас Microsoft
> отвечает на рукопожатие 403.** Сделано четыре попытки, каждая упиралась в
> одно и то же:
>
> | Попытка | Результат |
> |---|---|
> | Штатный WebSocket | Обрыв: нельзя задать `Origin` и `User-Agent` |
> | Своё рукопожатие поверх `Deno.connectTls` с нужными заголовками | 403 |
> | Версия `Sec-MS-GEC-Version` и `User-Agent` как в эталонном клиенте | 403 |
> | Добавлен параметр `ConnectionId` | 403 |
>
> Алгоритм токена `Sec-MS-GEC` сверен с эталонной реализацией посимвольно —
> совпадает. Рукопожатие доходит до сервера и получает честный HTTP-ответ,
> `Deno.connectTls` работает, TLS до `speech.platform.bing.com` открывается.
> То есть со стороны клиента отличий от эталона не осталось; вероятнее всего
> отбраковка идёт по адресу дата-центра или по отпечатку TLS, а его из Deno
> не подделать. **Дальше гадать смысла нет.**
>
> Поэтому звучит запасной движок — синтез Google Translate. Тоны стандартные
> пекинские, для отдельных знаков и коротких слов этого достаточно. Проверено
> не на глаз: выдача нашей функции и прямого запроса к Google совпала байт в
> байт (7296 байт, 24000 Гц, 64 кбит/с для 笔).
>
> **Как поднять качество, если понадобится.** Код нейронных голосов оставлен и
> включится без правок. Проще всего завести бесплатный ключ Azure Speech
> (500 000 знаков в месяц бесплатно) и задать секреты `AZURE_SPEECH_KEY`,
> `AZURE_SPEECH_REGION`, затем `TTS_PROVIDER=azure`. Голоса там те же самые
> нейронные, что и у Edge, но по ключу и без обходных путей.
