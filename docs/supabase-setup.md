...
> отвечает на рукопожатие 403** — судя по всему, блокирует адреса дата-центров:
> алгоритм токена `Sec-MS-GEC` сверен с эталонной реализацией посимвольно,
> рукопожатие доходит до сервера и получает честный HTTP-ответ. Поэтому
> фактически звучит запасной движок — синтез Google Translate. Тоны у него
> стандартные пекинские, для отдельных знаков и коротких слов этого достаточно.
> Проверено: выдача нашей функции и прямого запроса к Google совпала байт в
> байт (7296 байт, 24000 Гц, 64 кбит/с для 笔).
>
> Код нейронных голосов оставлен: если Microsoft перестанет отказывать или
> появится ключ Azure, качество поднимется без правок — достаточно поменять
> `TTS_PROVIDER`.
